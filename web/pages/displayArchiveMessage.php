...
$queryPrepared = $connect->prepare('SELECT m.idMessage,m.titre,m.texte,m.dateEnvoie,p1.nom FROM messagerie m INNER JOIN personne p1 ON p1.idPersonne = m.idSource WHERE m.serviceMessagerie = "majordhome" AND m.statutSource = 2 ORDER BY m.dateEnvoie desc;');
 $queryPrepared->execute();
 $array = $queryPrepared->fetchAll();


$queryPrepared = $connect->prepare('SELECT m.idMessage,m.titre,m.texte,m.dateEnvoie,p1.nom FROM messagerie m INNER JOIN personne p1 ON p1.idPersonne = m.idDestinataire WHERE m.serviceMessagerie IS NULL AND m.statutSource = 2 ORDER BY m.dateEnvoie desc;');
 $queryPrepared->execute();
