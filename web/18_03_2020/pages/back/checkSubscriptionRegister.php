...
$data = $connect->prepare("SELECT dateFin FROM souscription_abonnement WHERE statut = 0 AND FK_idPersonne = ".$_SESSION['idCustomer']);
$data->execute(array());
foreach ($data->fetchAll() as $key => $endTime){

    if ($endTime['dateFin'] > $dateTime){
        $_SESSION["subscription"] = 'Ce client possède déjà un abonnement';
        header('Location: subscriptionRegister.php');
    }
